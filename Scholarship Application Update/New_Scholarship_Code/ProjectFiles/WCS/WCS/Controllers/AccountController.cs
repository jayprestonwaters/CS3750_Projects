    ...
    public class AccountController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public async Task<IActionResult> Register(string code, string returnUrl = null)
        {
            if (code != null)
            {
                ...
                if (invite != null)
                {
                    if(!invite.Status.Equals("Expired") || invite.ExpirationDate >= DateTime.Now)
                    {
                        ViewData["ValidCode"] = true;
                        ViewData["Code"] = code;
                        ViewData["Email"] = invite.EmailAddress;
                    }
                    else
                    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (invite != null) //Has valid code
                    {
                        ...
                        if (!roleResult.Succeeded) _logger.LogInformation("Unable to add the role to the user.\n" + roleResult.Errors);
                        else _logger.LogInformation(user.Email.ToString() + " was added to the " + model.Role + " role.");
                    }
                    ...
                    _logger.LogInformation("Sent a confirmation email to the user.");

                    await _signInManager.SignInAsync(user, isPersistent: false); //TODO Remove so we don't log the user in right away until after they confirm their email.
                    _logger.LogInformation("User created a new account with password.");
                    return RedirectToAction("Index", "Home");
                }
                AddErrors(result);
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var user = await _userManager.FindByEmailAsync(model.Email);
                //TODO - Re-enable this code for production
